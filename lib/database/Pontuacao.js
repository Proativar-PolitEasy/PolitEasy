...
const RetornarPosicao = (idUsuario) => {
    const ref = database.ref("/pontuacoes").orderByChild("id_usuario").equalTo(idUsuario);
    ...
    return new Promise((resolve, reject) => {
        ...
        .then((snapshot) => {
            ...
            const id = Object.keys(snapshotValue)[0];
            const internalRef = database.ref("/pontuacoes").orderByChild("pontuacao").startAt(snapshotValue[id]["pontuacao"]);

            internalRef.once("value")
            .then((snapshotPontuacoes) => {
                const snapshotVal = snapshotPontuacoes.val();

                if (snapshotVal == null) {
                    return resolve(null);
                }

                resolve(Object.keys(snapshotVal).length);
            })
        })
        .catch((err) => {
            reject(err);
